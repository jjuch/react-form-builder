...
function (_React$Component) {
  ...
  function ReactForm(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "handleBack", function (e) {
      e.preventDefault();

      var data = _this._collectFormData(_this.props.data);

      _this.props.back_action(data);
    });
  ...
  _createClass(ReactForm, [{
    ...
    key: "_getCustomDefaultValue",
    value: function _getCustomDefaultValue(item, property) {
      var propertyValue;

      try {
        this.props.answer_data.forEach(function (el) {
    ...
    value: function _collect(item) {
      ...
      if (item.element === 'Checkboxes' || item.element === 'RadioButtons') {
        ...
        var itemValues = this._getItemValue(item, ref);

        itemData.value = itemValues.value;
        itemData.file = itemValues.file;
        itemData.creatorId = itemValues.creatorId;
      } else if (item.element === 'FileUpload') {
        var _itemValues = this._getItemValue(item, ref);

        itemData.value = _itemValues.value;
        itemData.file = _itemValues.file;
        itemData.creatorId = _itemValues.creatorId;
      } else {
        if (!ref) return null;
    ...
    value: function render() {
      ...
      var data_items = this.props.data.filter(function (el) {
        return el !== null;
      });
        ...
        switch (item.element) {
          ...
          case 'Download':
            return _react["default"].createElement(Download, {
              ...
              key: "form_".concat(item.id),
              data: item,
              getS3File: _this6.props.getS3File
            });
          ...
          case 'Camera':
            return _react["default"].createElement(Camera, {
              ...
              getS3File: _this6.props.getS3File,
              defaultValue: _this6._getDefaultValue(item),
              creatorId: _this6._getCustomDefaultValue(item, 'creatorId')
            });

          case 'FileUpload':
            return _react["default"].createElement(FileUpload, {
              ref: function ref(c) {
                return _this6.inputs[item.field_name] = c;
              },
              read_only: _this6.props.read_only || item.readOnly,
      ...
      return _react["default"].createElement("div", null, _react["default"].createElement(_formValidator["default"], {
        ...
        value: actionName
      }), !this.props.hide_actions && this.props.back_action && typeof this.props.back_action === "string" ? _react["default"].createElement("a", {
        href: this.props.back_action,
        className: "btn btn-default btn-cancel btn-big"
      }, backName) : _react["default"].createElement("a", {
